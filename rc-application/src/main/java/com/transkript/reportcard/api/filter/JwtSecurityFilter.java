...
// Reading Token from Authorization Header
        String tokenValue = request.getHeader(AppConstants.HEADER_STRING);
        if(tokenValue != null) {
            System.out.println(tokenValue);
            tokenValue = jwtUtil.removeTokenPrefix(tokenValue);
            JwtToken jwtToken = new JwtToken(tokenValue, jwtUtil.getClaims(tokenValue));
            String username = jwtToken.getSubject();
            //if username is not null & Context Authentication must be null
            if(username != null && SecurityContextHolder.getContext().getAuthentication()==null) {
                UserDetails user= userDetailsService.loadUserByUsername(username);
                boolean isValid= jwtToken.isValid( user.getUsername());
                if(isValid) {
